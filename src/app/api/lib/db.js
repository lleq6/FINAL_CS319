...
module.exports = {
    ...
    fetchAllCategory: async ()=>{
        ...
        const result = categoryL.rows.map(e => {
            return{...e,Sub_Category: subList.rows
                .filter((sub)=> sub.Category_ID == e.Category_ID)
                .map(sub => {
                    const t =  {
                        ...sub, 
                        ChildCategory : childList.rows.filter(child => child.Sub_Category_ID == sub.Sub_Category_ID)}
                    return t
                })}}
        )
        return result
    ...
    const result = categoryL.rows.map(e => {
        return{...e,Sub_Category: subList.rows
            ...
            .map(sub => {
                const t =  {
                    ...sub, 
                    child : childList.rows.filter(child => child.Sub_Category_ID == sub.Sub_Category_ID)}
                return t
            })}}
    )
    console.log(result[0].Sub_Category[0])
