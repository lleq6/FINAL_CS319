...
module.exports = {
    ...
    fetchProductCategory: async(id)=>{
        ...
        const data = await client.query(`SELECT * FROM public."Child_Sub_Category" WHERE "Child_ID" = $1`,[id]);
        // console.log(data.name);
        return data
    },
    fechProductCategoryID: async ()=>{
        const data = await client.query(`SELECT * FROM public."Child_Sub_Category"`)
        return data;
    },
    fetchAllCategory: async ()=>{
        ...
        const categoriesWithSubCategories = await Promise.all(categoryL.rows.map(async (Category) => {
            const subList = await client.query(`SELECT * FROM public."Sub_Category" WHERE "Category_ID" = $1`, [Category.Category_ID]);

            const subCategoriesWithChildren = await Promise.all(subList.rows.map(async (sub) => {
                const childList = await client.query(`SELECT * FROM public."Child_Sub_Category" WHERE "Sub_Category_ID" = $1`, [sub.Sub_Category_ID]);
                return {
                    ...sub,
    ...
    fetchProductsByCategory : async (category_id) => await client.query(`SELECT * FROM public."Product" WHERE "Child_ID" 
    IN (SELECT "Child_ID" FROM public."Child_Sub_Category" WHERE "Category_ID"=$1)`,[category_id]),

    fetchProductsBySubCategory : async (category_id) => await client.query(`SELECT * FROM public."Product" WHERE "Child_ID" 
    IN (SELECT "Child_ID" FROM public."Child_Sub_Category" WHERE "Sub_Category_ID"=$1)`,[category_id]),

    fetchProductsByChildCategory : async (category_id) => await client.query(`SELECT * FROM public."Product" WHERE "Child_ID" 
    IN (SELECT "Child_ID" FROM public."Child_Sub_Category" WHERE "Child_ID"=$1)`,[category_id]),
...
`,
    ...
    fetchSubCategoryDetail : async (Sub_ID) => await client.query(`
        SELECT 
            s.*,
			s."Name" as s_name,
            p."Name" as c_name
    ...
    insertToCart : async (id, product_id, Quantity) => await client.query(`INSERT INTO public."Cart_Detail" ("User_ID", "Product_ID", "Quantity") VALUES
    ($1, $2, $3)`, [id, product_id, Quantity]),

    fetchItemsInCart : async (user_id) => await client.query(`
        SELECT 
    ...
    addAddress : async ( Address ) => await client.query(`
        ...
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING "Address_ID";`,[
            Address.User_ID, Address.Address_1, Address.Address_2,
...
const a = async ()=>{
        const childCategory = await client.query(`SELECT * FROM public."Child_Sub_Category" WHERE "Child_ID" = $1`,[1])
        const subCategory = await client.query(`SELECT * FROM public."Sub_Category" WHERE "Sub_Category_ID" = $1`
            ,[childCategory.rows[0].Sub_Category_ID])
        const category = await client.query(`SELECT * FROM public."Category" WHERE "Category_ID" = $1`
