...
export async function DELETE(req: NextRequest, res: NextResponse) {
  ...
  try {
    if (data.keyword == "Category") {
      await query(
        `DELETE FROM "Sub_Category" WHERE "Category_ID"=${data.Category_ID}`
      );
      await query(
        `DELETE FROM "Child_Sub_Category" WHERE "Category_ID"=${data.Category_ID}`
      );
    }
    if (data.keyword == "Sub_Category") {
      await query(
        `DELETE FROM "Child_Sub_Category" WHERE "Sub_Category_ID"=${data.Sub_Category_ID}`
      );
    }
    await query(stringQuery, [data[Object.keys(data)[1] as keyof DeleteBody]]);
