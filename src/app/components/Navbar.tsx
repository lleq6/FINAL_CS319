...
const Navbar = () => {
    ...
    useEffect(()=>{
        ...
        function dropHandler(){
            const dropdown = document.querySelector('#category_menu');
            if (dropdown){dropdown.open = false}
        }
        function dropHandlerAdmin(){
            const dropdown = document.querySelector('#admin_menu');
            if (dropdown){dropdown.open = false}
        }
            ...
            return(
                ...
                {Categories.map( (Category : CategoryList) => 
                    <li key={Category.Category_ID}>
                        <details>
                            <summary><Link href={`/category/${Category.Category_ID}/`} onClick={dropHandler}>{Category.Name}</Link></summary>
                            <ul>
                                {Category.Sub_Category.map((Sub) =>
                                <li key={`${Category.Category_ID}${Sub.Sub_Category_ID}`}>
                                <details>
                                <summary><Link 
                                href={`/category/${Category.Category_ID}/${Sub.Sub_Category_ID}`} onClick={dropHandler}>{Sub.Name}</Link></summary>
                                <ul>
                                    {Sub.ChildCategory.map((Child) =>
                                        <li key={`${Category.Category_ID}${Sub.Sub_Category_ID}${Child.Child_ID}`}><Link href={`/category/${Category.Category_ID}/${Sub.Sub_Category_ID}/${Child.Child_ID}`} onClick={dropHandler}>{Child.Name}</Link></li>
                                    )}
                            </ul>
                            </details>
                            </li>
    ...
    return(
        <nav className="bg-gray-200">
            ...
            <div className="flex">
                ...
                <span className="mx-auto content-center"><Link href={'/contact'}>ติดต่อเรา</Link></span>
            </div>
            <div className="flex content-center">
                <Link href={'/'} className="content-center m-auto"><Image src={"/kacha-icon.png"} alt="test" width={80} height={80} className="m-auto"></Image></Link>
                ...
                <div className="content-center m-auto w-[20%]">
                    {!session.data ?
                    (<div className="flex content-center"><a className="link link-hover content-center px-2" onClick={()=> {document?.getElementById('my_modal_1').showModal()}}>ล็อคอิน / สมัครสมาชิก</a>
                    <GoTriangleDown className="my-auto"></GoTriangleDown></div>)
                    :
            ...
            <ul className="menu menu-horizontal flex-row px-1 p-0 w-11/12 mx-auto">
                ...
                <li><Link href={'/'} className="p-4 rounded-none">หน้าหลัก</Link></li>
                <li><Link href={'/products'} className="rounded-none p-4">สินค้าลดราคา</Link></li>
                ...
                <li className="bg-yellow-400 ml-auto mr-14">
                    ...
                    <summary className="rounded-none px-10 m-auto h-full font-semibold content-center">
                        <Link href={'/admin'} className="rounded-none px-7">Admin Menu</Link>
                    </summary>
                        <ul className="menu bg-base-200 rounded-box w-56 rounded-t-none">
                            <li>

                            </li>
                            <li><Link href={'/admin/user-management'} className="rounded-none p-4" onClick={dropHandlerAdmin}>จัดการสมาชิก</Link></li>
                            <li><Link href={'/admin/product-management'} className="rounded-none p-4" onClick={dropHandlerAdmin}>จัดการคลังสินค้า</Link></li>
                            <li><Link href={'/'} className="rounded-none p-4" onClick={dropHandlerAdmin}>จัดการคำสั่งซื้อ</Link></li>
                        </ul>
                    </details>
                    </li>:''
                }
