...
export default function AddressModal({ userAddresses, setAddresses, setCurAddress }: AddressModalProps) {
    const [address, setAddress] = useState<UserAddress | boolean>(false);
    const { data: session } = useSession();

    function AddressBox({ address, setCurAddress }: { address: UserAddress, setCurAddress: (address: UserAddress) => void }) {
        return (
            <div className={`grid grid-cols-[1fr_9fr_1fr] border border-1 rounded-lg p-2 my-2 content-center hover:border-green-500 ${address.Is_Default ? 'border-green-500 border-2' : ''}`} onClick={() => {
                ...
                <div className="flex flex-col content-center">
                    <a className="link link-hover flex text-end my-auto mx-3" onClick={(event) => {
            ...
            <dialog id="address-modal" className="modal">
                <div className="modal-box w-11/12 max-w-5xl">
                    ...
                    {userAddresses.map((address: UserAddress) => (
                        <AddressBox key={address.Address_ID} address={address} setCurAddress={setCurAddress} />
                    ))}
                    <div className="grid grid-cols-[9fr_1fr] border border-1 rounded-lg p-2 my-2 content-center py-5 hover:border-green-500 hover:text-green-500 text-center" onClick={() => {
                        setAddress(false);
                        document.getElementById('address-modal-edit').showModal();
                    }}>
