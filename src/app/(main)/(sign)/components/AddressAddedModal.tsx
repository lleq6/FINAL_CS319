...
import { generateOrderId } from '@/app/api/lib/utils'
...
export default function AddressEditModal({ Address, setAddress, setAddressList }: AddressEditModalProps) {
    ...
    useEffect(() => {
        ...
        if (!Address){
            setLocalAddress({
                ...
                Address_ID: 'adr-'+generateOrderId(),
                Address_1: '',
                Address_2: '',
                Province: '',
                District: '',
    ...
    function addHandler(e: SubmitEvent) {
        e.preventDefault();
        async function Add(){
            const response = await fetch('/api/user/addAddress', {method: 'POST', body: JSON.stringify(localAddress)})
            // console.log('adr-'+generateOrderId())
            // console.log(addres)
            if (response.ok){
                const { Address_ID } = await response.json()
                setAddressList(old => [...old,{...localAddress,Address_ID: Address_ID}])
    ...
    function editHandler(e: SubmitEvent) {
        e.preventDefault();
        async function Edit(){
            const response = await fetch('/api/user/editAddress', {method: 'POST', body: JSON.stringify(localAddress)})
            if (response.ok){
                setAddressList(old => old.map((e : UserAddress)=>{
    ...
    return (
        <div className="modal-box w-11/12 max-w-4xl [&>lebel]:mx-auto">
            ...
            <label className="form-control w-full max-w-[34rem] mx-auto">
                <ProvinceSelector address={localAddress} setAddress={setLocalAddress}></ProvinceSelector>
            </label>
            ...
            <form onSubmit={editHandler}>
                ...
                <div className="modal-action">
                    <button className="btn" type="submit">เพิ่มที่อยู่ใหม่</button>
                </div>
            </form>
                    }
            <form method="dialog">
