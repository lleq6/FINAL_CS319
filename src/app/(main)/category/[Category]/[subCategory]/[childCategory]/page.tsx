...
export default function childCategoryPage(){
    ...
    useEffect(()=>{
        async function getProduct(){
            const data = await fetch(`/api/products/getProductByChildCategory?category_id=${query.childCategory}`)
    ...
    return(
        <div>
            <h1>
                ...
                <Link href={`/category/${navDetail?.Category_ID}/${navDetail?.Sub_Category_ID}`}> {navDetail?.s_name} </Link>{'>'}
                <Link href={`/category/${navDetail?.Category_ID}/${navDetail?.Child_ID}`}> {navDetail?.cc_name} </Link></h1>
            <div className="flex">
